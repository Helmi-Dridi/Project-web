...
func UserRouterInit(router *gin.RouterGroup, db *gorm.DB) {
	...
	users := router.Group("/users/:companyID")
	{
		...
		// POST endpoint to upload a profile image for a specific user
		users.POST("/:ID/upload-profile-image", baseInstance.UploadProfileImage)
		users.GET("/admins/list", baseInstance.ReadAdminsList)
		users.GET("/admins/list/all", baseInstance.ReadAdminsListall)
		// ✅ Get users with no roles
		users.GET("/norole", baseInstance.ReadUsersWithNoRole)

		// ✅ Get users with 'Student' role
		users.GET("/students", baseInstance.ReadStudentsList)

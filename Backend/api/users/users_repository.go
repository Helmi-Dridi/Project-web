...
// UpdateProfilePicture updates the profile picture path for a specific user in the database.
func UpdateProfilePicture(db *gorm.DB, userID uuid.UUID, profilePicturePath string) error {
	// Find the user and update the profile picture path
	return db.Model(&domains.Users{}).Where("id = ?", userID).Update("profile_picture", profilePicturePath).Error
}
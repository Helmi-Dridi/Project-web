...
// CreateNotification handles the creation of a notification.
// @Summary         Create a notification
// @Description     Add a new notification for the user
// @Tags            Notifications
// @Accept          json
// @Produce         json
// @Security        ApiKeyAuth
// @Param           userID   path      string                   true  "User ID"
// @Param           request  body      notifications.NotificationsIn  true  "Notification data"
// @Success         200      {object}  utils.ApiResponses
...
func (db Database) CreateNotification(ctx *gin.Context) {
	session := utils.ExtractJWTValues(ctx)
	...
	// Check if the user is allowed to create notifications for this user
	if err := domains.CheckEmployeeSession(db.DB, userID, session.UserID, session.CompanyID); err != nil {
		logrus.Error("User not authorized: ", err)
		utils.BuildErrorResponse(ctx, http.StatusForbidden, constants.INVALID_REQUEST, utils.Null())
		return
	}

	// Parse request body
	var input NotificationsIn
	if err := ctx.ShouldBindJSON(&input); err != nil {
		logrus.Error("Failed to parse notification input: ", err)
		utils.BuildErrorResponse(ctx, http.StatusBadRequest, constants.INVALID_REQUEST, utils.Null())
		return
	}
	...
	notif := &domains.Notifications{
		...
		Seen:    input.Seen, // Optional (false by default)
		UserID:  userID,
	}

	// Save to DB
	if err := domains.Create(db.DB, notif); err != nil {
		logrus.Error("Failed to create notification: ", err)
		utils.BuildErrorResponse(ctx, http.StatusInternalServerError, constants.UNKNOWN_ERROR, utils.Null())
		return
	}

	utils.BuildResponse(ctx, http.StatusOK, constants.SUCCESS, notif)
}
